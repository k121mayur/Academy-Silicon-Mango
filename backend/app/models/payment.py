...
class PaymentSettings(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    # Only the ACTIVE mode is stored here. Razorpay secrets live in the backend
    # env (settings.RAZORPAY_TEST_*/LIVE_*), never in the DB — so a DB dump or
    # backup can never leak payment credentials.
