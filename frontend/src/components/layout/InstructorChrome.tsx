...
const NAV: NavItem[] = [
  { label: "Dashboard", to: "/instructor/dashboard", icon: "space_dashboard" },
  { label: "Assigned Batches", to: "/instructor/batches", icon: "groups_2" },
  { label: "Create Assignment", to: "/instructor/assignments/new", icon: "assignment_add" },
  { label: "Course Plan", to: "/instructor/plan", icon: "calendar_view_week" },
  { label: "Sessions & Resources", to: "/instructor/sessions", icon: "event" },
  { label: "Attendance", to: "/instructor/attendance", icon: "fact_check" },
  { label: "Submissions & Grading", to: "/instructor/grading", icon: "grading" },
  { label: "Completion", to: "/instructor/completion", icon: "workspace_premium" },
];

export function InstructorSidebar({ onNavigate }: { onNavigate?: () => void }) {
  return (
    <aside className="w-64 h-full bg-surface-lowest border-r border-ink-outlineVariant/40 shadow-nav flex flex-col">
      ...
      >
        ...
        <div className="leading-tight">
          ...
          <p className="text-label text-ink-outline">Instructor Panel</p>
        </div>
      </Link>
